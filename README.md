...
> **Note:** The backend uses MongoDB. If no `MONGODB_URI` is provided in `server/.env`, it will gracefully fall back to local JSON storage (`server/data/bone_health_store.json`), allowing the app to run for demos and local testing without a database.
