        ...
        <div className="relative pointer-events-none select-none">
          ...
          >
            <path
              ...
              fill="none"
              stroke={pinColor}
              strokeWidth={isElevated ? '1.4' : '0.8'}
          ...
          >
            <div
              className={`rounded-lg transition-all ${
                isElevated ? 'scale-105 ring-1 ring-cyan-500/50 shadow-2xl' : 'hover:scale-102 shadow-sm'
              }`}
              style={{
                background: isElevated ? 'rgba(2, 6, 23, 0.96)' : 'rgba(9, 13, 31, 0.92)',
                border: `1px solid ${isElevated ? pinColor : pinColor + '88'}`,
                boxShadow: isElevated
                  ? `0 6px 18px rgba(0,0,0,0.95), 0 0 10px ${pinColor}66`
                  : '0 1px 4px rgba(0,0,0,0.5)',
                width: isElevated ? 120 : 'max-content',
                minWidth: 60,
                maxWidth: isElevated ? 135 : 85,
                padding: isElevated ? '3px 6px' : '2px 4px',
              }}
            >
              {/* Header: Full Zone Name & Status Badge */}
              <div className="flex items-center justify-between gap-1">
                <span className="font-black text-white whitespace-nowrap text-[8px] tracking-tight truncate max-w-[80px]">
                  {z.label}
                </span>
                <span
                  className="font-black px-1 py-0.5 rounded shrink-0 uppercase text-[6px]"
                  style={{
              ...
              <div className="flex items-center justify-between gap-1 mt-0.5 text-slate-400">
                <span className="font-semibold text-[6.5px] whitespace-nowrap text-slate-300 truncate max-w-[65px]">
                  {z.subLabel || 'Landmark'}
                </span>
                <span className="font-mono font-bold text-[7px] shrink-0" style={{ color: pinColor }}>
                  T: {z.tScore}
                </span>
              </div>
              ...
              {isElevated && (
                <div className="mt-1.5 pt-1 border-t border-white/15 text-left animate-fade-in space-y-1">
                  <div className="text-[6.8px] font-black uppercase tracking-wider text-cyan-400">
                    Risk Zone Insight:
                  </div>
                  <p className="text-[7.5px] text-slate-200 leading-snug font-medium line-clamp-3 break-words">
                    {z.note}
                  </p>
                  <div className="flex items-center justify-between text-[7px] bg-slate-900/90 px-1.5 py-0.5 rounded border border-slate-800 mt-1">
...
function AnnotationPins({ zones, activeZoneId, hoveredZoneId, showAnnotations, onSelectZone, onHoverZone }) {
  const zonesToRender = showAnnotations
    ? zones
    : hoveredZoneId
      ? zones.filter(z => z.id === hoveredZoneId || z.canonicalId === hoveredZoneId)
      : [];

  if (!zonesToRender.length) return null;
    ...
    <group>
      {zonesToRender.map((z) => (
        <AnnotationPinItem
          key={z.id}
          z={z}
          isSelected={activeZoneId === z.id || activeZoneId === z.canonicalId}
          isHovered={hoveredZoneId === z.id || hoveredZoneId === z.canonicalId}
...
function RealAnatomicalBoneModel({
  ...
  const getZoneAtEvent = useCallback((e) => {
    if (!e || !zones.length) return { zone: null };
    const direct = findZoneByMeshName(e.object?.name, zones);
    let nearest = null;
    if (e.point && rootRef.current) {
      const lp = rootRef.current.worldToLocal(e.point.clone());
      nearest = zones.reduce((best, z) => {
        const d = lp.distanceTo(new THREE.Vector3(...z.anchor));
        return !best || d < best.d ? { z, d } : best;
      }, null)?.z;
    }
    return { zone: direct || nearest || zones[0], point: e.point ? e.point.toArray() : null };
  }, [zones]);

  const activeHoverId = typeof hoveredZone === 'string' ? hoveredZone : hoveredZone?.id;
  ...
  return (
    <group
      ...
      onPointerMove={e => {
        e.stopPropagation();
        const r = getZoneAtEvent(e);
        if (r.zone) onZoneEvent?.({ ...r, type: 'hover' });
      }}
      onPointerLeave={() => onZoneEvent?.({ type: 'out' })}
      onClick={e => {
        e.stopPropagation();
        const r = getZoneAtEvent(e);
        if (r.zone) onZoneEvent?.({ ...r, type: 'click' });
      }}
    >
...
export default function BoneModelViewer({
  ...
  const handleZoneEvent = useCallback((ev) => {
    ...
    if (!ev || ev.type === 'out') {
      // 140ms grace period so moving between bone mesh and HTML badge doesn't flicker
  ...
  return (
    <div
      className="relative h-full w-full min-w-0 max-w-full overflow-hidden select-none"
      onPointerLeave={() => handleZoneEvent({ type: 'out' })}
    >
      <Canvas
